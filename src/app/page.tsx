    ...
    <main className="min-h-screen">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 glass">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <h1 className="text-2xl font-serif text-primary">Alquiler temporal Mendoza</h1>
          <div className="hidden md:flex items-center gap-8">
            <a href="#cabins" className="text-gray-700 hover:text-primary transition-colors">Nuestras Quintas</a>
            <a href="#features" className="text-gray-700 hover:text-primary transition-colors">Servicios</a>
            {user ? (
              <>
                {isAdmin && (
                  <Link href="/admin" className="text-primary hover:text-opacity-80 transition-colors font-semibold flex items-center gap-1">
                    Panel Admin
                  </Link>
                )}
        ...
        {mobileMenuOpen && (
          ...
          >
            <a href="#cabins" className="block text-gray-700">Nuestras Quintas</a>
            <a href="#features" className="block text-gray-700">Servicios</a>
            {user && isAdmin && (
              <Link href="/admin" className="block text-primary font-bold">
                Panel Admin
              </Link>
            )}
            {user ? (
              <>
                <Link href="/bookings" className="block text-gray-700 font-medium">
                  Mis Reservas
                </Link>
                <button 
                  onClick={handleSignOut} 
                  className="block text-gray-700 w-full text-left font-medium cursor-pointer"
                >
                  Cerrar Sesión
                </button>
              </>
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        >
          <h1 className="text-6xl md:text-8xl font-serif mb-6 drop-shadow-lg">Alquiler temporario de cabañas</h1>
          <p className="text-xl md:text-2xl font-light mb-8 opacity-90 tracking-widest">EL LUGAR PERFECTO PARA TUS CUMPLEAÑOS Y EVENTOS BAJO EL SOL</p>
          <a href="#cabins" className="inline-flex bg-primary hover:bg-opacity-90 text-white px-10 py-4 rounded-full text-lg font-medium transition-all transform hover:scale-105 shadow-2xl items-center gap-2">
            Explorar Quintas y Cabañas <ArrowRight size={20} />
          </a>
        </motion.div>
      ...
      <section id="features" className="py-24 bg-white px-6">
        <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-12 text-center">
          {[
            { icon: Calendar, title: "Reservas para Eventos", desc: "Calendario intuitivo para asegurar la fecha de tu festejo." },
            { icon: ShieldCheck, title: "Espacios Equipados", desc: "Quinchos completos, parrillas, piletas y todo lo necesario para tu cumpleaños." },
            { icon: Trees, title: "Entorno Mendocino", desc: "Disfrutá del aire libre rodeado de la tranquilidad y arboledas de Mendoza." },
          ].map((feature, index) => (
            <motion.div 
              key={index}
              initial={{ opacity: 0, y: 20 }}
      ...
      <section id="cabins" className="py-24 bg-gray-50 px-6">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl md:text-5xl font-serif text-center mb-16">Nuestras Quintas y Cabañas</h2>
          
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              <Loader2 className="animate-spin text-primary" size={48} />
              <p className="text-gray-500 font-serif">Buscando las mejores quintas mendocinas...</p>
            </div>
          ) : error ? (
            ...
            <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200">
              ...
              <p className="text-gray-500 text-xl font-serif">Aún no hay quintas o cabañas disponibles.</p>
      ...
      <footer className="bg-primary text-white py-16 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="text-center md:text-left">
            <h2 className="text-3xl font-serif mb-2">Alquiler temporal mendoza</h2>
            <p className="opacity-70">Las mejores opciones para tus cumpleaños y escapadas de verano.</p>
          </div>
          ...
          <div className="text-sm opacity-50">
            © 2026 Quintas & Cabañas Mendoza. Todos los derechos reservados.
          </div>
        </div>
      </footer>
    </main>
